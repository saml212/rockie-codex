...
neither `${OPENCLAW_WORKSPACE_DIR}/gpu-custom.md` exists nor
...
> (`ROCKIE_GPU_MODE=custom` then `/gpu-custom-setup` walks your flow).
...
`ROCKIE_GPU_MODE=custom`; the next `/experiment` invocation will see
the env and trigger `/gpu-custom-setup` for the one-time flow audit.
